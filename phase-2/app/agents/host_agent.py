...
async def check_alert_status(ctx: Context) -> str:
    ...
    try:
        await sleep(60)  # Give Alertmanager time to update
...
async def store_alert_diagnostics(
    ctx: Context,
    alert_name: str,
    namespace: str,
    alert_diagnostics: str,
) -> str:
    """Store alert diagnostics in context for other agents to access.
...
STEP 0: Call store_alert_diagnostics FIRST
- Input: alert_name (str), namespace (str), alert_diagnostics (str)
...
tools = [
    FunctionTool.from_defaults(
        fn=store_alert_diagnostics,
        name="store_alert_diagnostics",
        description="""Store alert diagnostics in shared context for other agents to access.
